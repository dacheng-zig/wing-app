...
文档质量完全取决于签名表达力。用 `wing.extract.Path/Query/Json` 接收输入、用 `wing.respond.Json/Created` 返回，文档才能推导出参数与 schema：

```zig
// src/user/controllers/user_controller.zig
pub fn index(ctx: *Ctx, svc: *UserService) anyerror!wing.respond.Json([]User) { ... }   // → 200 + User[] schema

pub fn show(
    ctx: *Ctx,
    svc: *UserService,
    path: wing.extract.Path(struct { id: u64 }),                                 // → path 参数 id
) anyerror!wing.respond.Json(User) { ... }                                      // → 200 + User schema

pub fn create(
    ctx: *Ctx,
    svc: *UserService,
    body: wing.extract.Json(CreateUserReq),                                     // → requestBody
) anyerror!wing.respond.Created(User) { ... }                                  // → 201 + User schema
```
...
| 你写的 | 文档里变成 |
|---|---|
| `wing.extract.Path(struct{ id: u64 })` | path 参数（必填） |
| `wing.extract.Query(struct{ page: ?u32 })` | query 参数（optional/有默认 → 非必填） |
| `wing.extract.Json(T)` 入参 | `application/json` 请求体 |
| `wing.respond.Json(T)` 返回 | `200` + T 的 schema |
| `wing.respond.Created(T)` 返回 | `201` + T 的 schema |
| `wing.respond.Redirect` 返回 | `302` |
...
A：不会。指针型（`*State`/`*Service`）自动忽略；`wing.extract.Path/Query/Json` 进参数/请求体；`Auth` 等鉴权 extractor 进 `security`（自动）。
